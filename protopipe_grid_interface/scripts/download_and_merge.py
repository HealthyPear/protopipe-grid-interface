...
def main():
    ...
    if not metadata:
        ...
        analysis_path_local = local / "shared_folder/analyses" / args.analysis_name
    ...
    for part in args.particle_types:
        ...
        if not args.disable_download:
            log.info(f"Downloading {part}")
            log.debug(f"...from {input_directory}")
            log.debug(f"...to {output_directory}")
            download(str(input_directory), str(output_directory))
            n_files = len(glob.glob(str(output_directory / "*.h5")))
            log.info(f"{n_files} files have been downloaded into {output_directory}")
        ...
        if not args.disable_merge:
            ...
            merge_call(template_file_name, output_directory, output_file)

            log.info(f"Downloaded files have been merged into {output_file}")
